...
class AutomatedTest4 {
	...
	//scenario 4 - Two client 1st requests with 2 reqParams and 2nd requests with 2 reqParams
	//verify 5000ms < responseTime > 10000ms (queueBatchSize<5  queue cap not full)
	...
	public void automatedTest() throws InterruptedException {
		...
		new Thread(new Runnable() {
			public void run() {
				...
				Response response1 = httpRequest1.get("http://localhost:8080/aggregation?pricing=PP,PQ&track=123456891,109347292&shipments=109347292,109347293");
				String res1 = response1.asString();
				ValidatableResponse validatableResp1 = response1.then();
				validatableResp1.time(Matchers.lessThan(10000L));
				validatableResp1.time(Matchers.greaterThan(5000L));
		...
		new Thread(new Runnable() {
			public void run() {
				...
				validatableResp2.time(Matchers.lessThan(10000L));
				validatableResp2.time(Matchers.greaterThan(5000L));
